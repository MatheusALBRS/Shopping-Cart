...
const Products = (props) => {
  ...
  const deleteButtonHandler = (cartItem) => {
  ...
  useEffect(() => {
    const fetchPhotos = () => {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        let url = `https://picsum.photos/id/${number}/200/`;
        list.push(url);
      }
      return list;
    };
  ...
  const restockProducts = (url, data) => {
    doFetch(url);
    const newProducts = data.data.map((item) => item.attributes);
